...
//	 * BLACK_KING.
//	 */
//	private void setPieceAt(int row, int col, int piece) {
//	   board[row][col] = piece;
//	}
	...
	protected CheckersMove[] getLegalJumpsFrom(int player, int row, int col) {
	   ...
	   if (moves.size() == 0)
	      ...
	      for (int i = 0; i < moves.size(); i++) {
	         moveArray[i] = moves.get(i);
             System.out.println("Contents of getLegalJumps moveArray = " + moveArray[i].getFromRow() + "," 
            		 + moveArray[i].getFromCol()+ " --> " + moveArray[i].getToRow() + "," + moveArray[i].getToCol());
             System.out.println("----------------------------------------------------");
	      }
	      return moveArray;
	   }
	}//end method getLegalJumpsFrom()
